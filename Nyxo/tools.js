...
const { getMcpToolSchema, executeMcpTool } = require("./mcp");
...
/**
 * Returns the full tool schema: built-ins + any connected MCP tools.
 * Called dynamically per-prompt so MCP tools appear as soon as connected.
 */
function getFullSchema() {
  return BUILTIN_SCHEMA + getMcpToolSchema();
}

// Keep TOOL_SCHEMA as alias so nothing breaks if imported elsewhere
const TOOL_SCHEMA = BUILTIN_SCHEMA;
...
/**
 * Execute a tool by name. Async to support MCP tool calls.
 * MCP tools are identified by the prefix "mcp__serverName__toolName".
 */
async function executeTool(name, args) {
  ...
  if (name.startsWith("mcp__")) {
    ...
    if (parts.length < 3) {
      return `Error: invalid MCP tool format "${name}". Expected: mcp__serverName__toolName`;
    }
    const serverName = parts[1];
    const toolName   = parts.slice(2).join("__"); // handle tool names that contain __
  ...
  const fn = TOOLS[name];
  if (!fn) {
    return `Unknown tool: "${name}". Built-ins: ${Object.keys(TOOLS).join(", ")}. MCP tools use the mcp__server__tool format.`;
  }
  return fn(args);
}
