...
// ─────────────────────────────────────────────────────────────────────────────
// /api/create-vps — spin up a VPS container without a GitHub repo
// Creates the ports.json entry AND the Docker container in one shot.
// The container uses `tail -f /dev/null` as PID 1 so it stays alive persistently.
// Use /api/fs/* to add files, then /api/redeploy to run them.
// One-time only: if the container already exists it returns an error.
...
app.post("/api/create-vps", async (req, res) => {
  ...
  const nameCheck = validateAppName(appName);
  if (!nameCheck.ok) return res.json({ status: "error", message: nameCheck.msg });
  ...
  // Check if a Docker container already exists — if yes, nothing to create
  try {
  ...
  const existingEntry = data[appName] || null;
  let port, ram, cpu, storage;

  if (existingEntry) {
    // App already in ports.json — validate ownership and password before reusing
    if (!user.servers.includes(appName)) {
      return res.json({ status: "error", message: "You don't own this app" });
    }
    if (existingEntry.password !== appPassword) {
      return res.json({ status: "error", message: "Invalid app password" });
    }

    // Reuse existing specs (port, plan, ram, cpu, etc.)
    port    = parseInt(existingEntry.port.split(":")[1]);
    ram     = existingEntry.ram     || FREE_RAM;
    cpu     = existingEntry.cpu     || FREE_CPU;
    storage = existingEntry.storage || FREE_STORAGE;
  } else {
    // Brand-new app — make sure the name isn't taken by someone else
    const nameCheck = validateAppName(appName);
    if (!nameCheck.ok) return res.json({ status: "error", message: nameCheck.msg });

    // Assign a free port and create a fresh entry
    const usedPorts = Object.values(data).map(v => parseInt(v.port.split(":")[1]));
    port = 40000 + Object.keys(data).length;
  ...
  // Spin up the container (queued, same flags as deploy)
  try {
    await runQueued(async () => {
    ...
    // Rollback only if we created a fresh entry (don't wipe existing premium data)
    await safeRun("docker", ["rm", "-f", appName]).catch(() => {});
    if (!existingEntry) {
      delete data[appName];
      saveData(data);
