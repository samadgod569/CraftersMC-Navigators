...
app.post("/api/signup", async (req, res) => {
  const { code } = req.body;
  // nginx is on localhost so X-Real-IP = 127.0.0.1; the real client IP is in X-Forwarded-For
...
const tmpImage = `cloudra_scan_${appName}_${Date.now()}`.toLowerCase();
  try {
    // Commit the running container to a temp local image so trivy can scan it
    await safeRun("docker", ["commit", appName, tmpImage]);
    ...
    safeRun("docker", ["rmi", "-f", tmpImage]).catch(() => {});
  }
});
