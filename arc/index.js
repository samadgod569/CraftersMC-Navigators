...
function buildDockerfile(image, language, packages, startFile) {
  let df = `FROM ${image}\nWORKDIR /app\nCOPY . .\n\nRUN useradd -m appuser 2>/dev/null || adduser -D appuser\n`;
  ...
  if (language.startsWith("nodejs") || language === "ts" || language.startsWith("ts")) {
    ...
    packages?.forEach(p => df += `RUN mix local.hex --force && mix archive.install hex ${p} --force\n`);
  ...
  df += `RUN chown -R appuser:appuser /app\n`;
  df += `USER appuser\n`;
