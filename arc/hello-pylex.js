 ...
 * Lets the user customize panel colors, set a custom background image,
 * and switch the File Manager editor between Monaco and Ace.
...
(function () {
  ...
  const ACE_EXT_CDNS = [
    'https://cdnjs.cloudflare.com/ajax/libs/ace/1.32.6/ext-language_tools.js',
    'https://cdnjs.cloudflare.com/ajax/libs/ace/1.32.6/ext-searchbox.js',
    'https://cdnjs.cloudflare.com/ajax/libs/ace/1.32.6/ext-beautify.js',
  ];
  const CODEMIRROR_CDN = 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.16/codemirror.min.js';
  const CODEMIRROR_CSS = 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.16/codemirror.min.css';
  const CODEMIRROR_THEME_CSS = {
    light: 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.16/theme/eclipse.min.css',
    dark: 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.16/theme/material-darker.min.css',
  };
  const CODEMIRROR_ADDONS = [
  ...
  const CODEMIRROR_HINT_CSS = 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.16/addon/hint/show-hint.min.css';
  const CODEMIRROR_MODES = {
    javascript: 'javascript', typescript: 'javascript', python: 'python',
    html: 'htmlmixed', css: 'css', scss: 'css', less: 'css', json: 'javascript',
    markdown: 'markdown', shell: 'shell', go: 'go', ruby: 'ruby',
    php: 'php', rust: 'rust', java: 'clike', kotlin: 'clike', swift: 'swift',
    yaml: 'yaml', xml: 'xml', sql: 'sql', plaintext: null, csharp: 'clike',
    cpp: 'clike', lua: 'lua', r: 'r', dockerfile: 'dockerfile', ini: 'properties',
    graphql: 'javascript', proto: 'protobuf', makefile: null,
  };
  ...
  const EDITOR_MODES = { monaco: 'Monaco (default)', ace: 'Ace', codemirror: 'CodeMirror' };
  ...
  function ensureBgStyleTag() {
    let tag = document.getElementById('themes-ext-bg-style');
    if (!tag) {
      tag = document.createElement('style');
      tag.id = 'themes-ext-bg-style';
  ...
  function ensureBgLayer() {
    let layer = document.getElementById('themes-ext-bg-layer');
    if (!layer) {
      layer = document.createElement('div');
      layer.id = 'themes-ext-bg-layer';
      // Insert as the very first child of <body> so it sits behind
      // everything and is NOT a flex item affecting layout (it's
      // taken out of flow via position:fixed, but some browsers still
      // generate box geometry for flex children before going out-of-flow —
      // appending as first child with explicit top/left/width/height avoids that).
      document.body.insertBefore(layer, document.body.firstChild);

      const dimLayer = document.createElement('div');
      dimLayer.id = 'themes-ext-bg-dim-layer';
      document.body.insertBefore(dimLayer, document.body.firstChild.nextSibling);
    }
    return layer;
  }
  ...
  function applyBackground() {
    ...
    if (!bg || !bg.url) {
      tag.textContent = '';
      const layer = document.getElementById('themes-ext-bg-layer');
      const dimLayer = document.getElementById('themes-ext-bg-dim-layer');
    ...
    const dim = typeof bg.dim === 'number' ? bg.dim : 0;
    ...
    layer.style.cssText = `
      ...
      margin: 0; padding: 0;
      flex: none !important;
      background-image: url("${bg.url.replace(/"/g, '\\"')}");
    ...
    tag.textContent = `
      ...
      body > *:not(#themes-ext-bg-layer):not(#themes-ext-bg-dim-layer) {
        position: relative;
        z-index: 1;
      }
      .main, .sidebar, nav {
        background-color: color-mix(in srgb, var(--bg) ${Math.round((1 - (bg.panelTransparency || 0)) * 100)}%, transparent) !important;
      }
      .surface, .inspect-card, .usage-stat-card, .backup-card, .ext-card,
      .terminal-wrap, .danger-zone, [style*="var(--surface)"] {
  ...
  function loadAceScript() {
    ...
    _aceLoading = (async () => {
      if (!window.ace) await loadScript(ACE_CDN);
      // Load extensions sequentially after core (they register onto window.ace)
      for (const url of ACE_EXT_CDNS) {
        try { await loadScript(url); } catch (e) { console.warn('[Themes Extension]', e); }
      }
    })();
    return _aceLoading;
  }

  let _codemirrorLoading = null;
  function loadCodeMirrorScript() {
    if (_codemirrorLoading) return _codemirrorLoading;
    _codemirrorLoading = (async () => {
      loadCss(CODEMIRROR_CSS);
      loadCss(CODEMIRROR_HINT_CSS);
      if (!window.CodeMirror) await loadScript(CODEMIRROR_CDN);
      for (const url of CODEMIRROR_ADDONS) {
        try { await loadScript(url); } catch (e) { console.warn('[Themes Extension]', e); }
      }
      // Load language modes on demand via mode/meta + multiplex; for simplicity
      // preload the common ones.
      const modeUrls = [
        'mode/javascript/javascript', 'mode/python/python', 'mode/xml/xml',
        'mode/htmlmixed/htmlmixed', 'mode/css/css', 'mode/markdown/markdown',
        'mode/clike/clike', 'mode/shell/shell', 'mode/go/go', 'mode/ruby/ruby',
        'mode/php/php', 'mode/rust/rust', 'mode/swift/swift', 'mode/yaml/yaml',
        'mode/sql/sql', 'mode/lua/lua', 'mode/r/r', 'mode/dockerfile/dockerfile',
        'mode/properties/properties',
      ];
      for (const m of modeUrls) {
        try { await loadScript(`https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.16/${m}.min.js`); }
        catch (e) { /* ignore missing modes */ }
      }
    })();
    return _codemirrorLoading;
  }
  ...
  function getCurrentEditorValue() {
    try {
      if (window._aceEditor) return window._aceEditor.getValue();
    } catch (e) {}
    return '';
  }
  ...
  async function switchToAce() {
    ...
    const cmHost = document.getElementById('themes-ext-cm-host');
    ...
    // Ctrl+Space -> trigger autocomplete explicitly (in addition to live autocompletion)
    editor.commands.addCommand({
      name: 'pylexTriggerAutocomplete',
  ...
  async function switchToCodeMirror() {
    const container = document.getElementById('fm-monaco-container');
    if (!container) return;

    await loadCodeMirrorScript();
    ...
    let cmHost = document.getElementById('themes-ext-cm-host');
    if (!cmHost) {
      cmHost = document.createElement('div');
      cmHost.id = 'themes-ext-cm-host';
      cmHost.style.cssText = 'position:absolute; inset:0; overflow:auto;';
      container.style.position = 'relative';
      container.appendChild(cmHost);
    }
    cmHost.style.display = 'block';

    const isDark = document.documentElement.getAttribute('data-theme') === 'dark';

    let cm = window._codeMirrorInstance;
    if (!cm) {
      cmHost.innerHTML = '';
      cm = window.CodeMirror(cmHost, {
        ...
        extraKeys: {
          ...
          'Ctrl-S': function () { if (typeof window.fmSaveFile === 'function') window.fmSaveFile(); return false; },
          'Cmd-S': function () { if (typeof window.fmSaveFile === 'function') window.fmSaveFile(); return false; },
        },
      });
      window._codeMirrorInstance = cm;

      cm.on('inputRead', function (instance, change) {
        if (!instance.state.completionActive && change.text[0] && /[\w.]/.test(change.text[0])) {
          instance.showHint({ completeSingle: false, hint: window.CodeMirror.hint.anyword });
        }
      });

      if (!cm._themesExtResizeObserver) {
        const ro = new ResizeObserver(() => cm.refresh());
        ro.observe(container);
        cm._themesExtResizeObserver = ro;
      }
      if (!cm._themesExtThemeObserver) {
        const observer = new MutationObserver(() => {
          const dark = document.documentElement.getAttribute('data-theme') === 'dark';
          cm.setOption('theme', dark ? 'material-darker' : 'eclipse');
        });
        observer.observe(document.documentElement, { attributes: true, attributeFilter: ['data-theme'] });
        cm._themesExtThemeObserver = observer;
      }
    } else {
      cm.setValue(currentValue);
    }

    if (!_monacoShim) _monacoShim = window._aceEditor;

    window._aceEditor = {
      getValue: () => cm.getValue(),
      setValue: (v) => { cm.setValue(v == null ? '' : v); cm.clearHistory(); cm.focus(); },
      focus: () => cm.focus(),
      clearSelection: () => cm.setCursor(0, 0),
      scrollToLine: () => cm.scrollTo(0, 0),
      session: {
        setMode: (aceMode) => {
          const m = aceMode.replace('ace/mode/', '');
          const cmMode = CODEMIRROR_MODES[m];
          cm.setOption('mode', cmMode || null);
        }
      }
    };
  ...
  function applyEditorMode() {
    ...
    const tryApply = () => {
      ...
      } else if (mode === 'codemirror') {
        switchToCodeMirror().catch(e => console.warn('[Themes Extension] CodeMirror load failed:', e));
      } else {
        if (window._monacoEditor) switchToMonaco();
      }
    };
    if (window._monacoEditor || mode !== 'monaco') {
      tryApply();
    } else {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .te-row-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
      ...
      @media (max-width: 480px) {
        ...
        .te-row-2 { grid-template-columns: 1fr; }
      }
    `;
    ...
    return `
      <div class="te-header">
        <div>
          ...
          <div class="te-sub">Customize colors, background &amp; editor — saved to this browser</div>
        </div>
      ...
      <div class="te-body">
        ...
        <div class="te-section">
          <div class="te-section-title">Custom Background</div>
          <div class="te-field">
            <label>Image URL</label>
            <input type="text" id="te-bg-url" placeholder="https://example.com/wallpaper.jpg" value="${escAttr(bg.url || '')}" />
          </div>
          <div class="te-row-2">
            <button class="te-btn" id="te-bg-upload-btn">
              <svg viewBox="0 0 24 24" fill="none"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/><polyline points="17 8 12 3 7 8" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/><line x1="12" y1="3" x2="12" y2="15" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>
              Upload Image
            </button>
            <input type="file" id="te-bg-file" accept="image/*" style="display:none;" />
            <button class="te-btn te-btn-danger" id="te-bg-clear-btn">
          ...
          <div class="te-field" style="margin-top:12px;">
            <label>Image Opacity</label>
            <div class="te-range-row">
              <input type="range" id="te-bg-opacity" min="0" max="1" step="0.05" value="${typeof bg.opacity === 'number' ? bg.opacity : 1}" />
              <span id="te-bg-opacity-val">${Math.round((typeof bg.opacity === 'number' ? bg.opacity : 1) * 100)}%</span>
            </div>
          </div>
          <div class="te-field">
            <label>Dim Overlay</label>
            <div class="te-range-row">
              <input type="range" id="te-bg-dim" min="0" max="0.9" step="0.05" value="${typeof bg.dim === 'number' ? bg.dim : 0}" />
              <span id="te-bg-dim-val">${Math.round((typeof bg.dim === 'number' ? bg.dim : 0) * 100)}%</span>
            </div>
          </div>
          <div class="te-field">
            <label>Blur</label>
            <div class="te-range-row">
              <input type="range" id="te-bg-blur" min="0" max="20" step="1" value="${typeof bg.blur === 'number' ? bg.blur : 0}" />
              <span id="te-bg-blur-val">${typeof bg.blur === 'number' ? bg.blur : 0}px</span>
            </div>
          </div>
          <div class="te-field">
            <label>Panel Transparency (nav / sidebar / main)</label>
            <div class="te-range-row">
              <input type="range" id="te-bg-paneltrans" min="0" max="0.9" step="0.05" value="${typeof bg.panelTransparency === 'number' ? bg.panelTransparency : 0}" />
              <span id="te-bg-paneltrans-val">${Math.round((typeof bg.panelTransparency === 'number' ? bg.panelTransparency : 0) * 100)}%</span>
            </div>
          </div>
          <div class="te-row-2">
            <div class="te-field">
              <label>Background Size</label>
              <select id="te-bg-size">
                <option value="cover" ${(bg.size||'cover')==='cover'?'selected':''}>Cover</option>
                <option value="contain" ${bg.size==='contain'?'selected':''}>Contain</option>
                <option value="auto" ${bg.size==='auto'?'selected':''}>Auto / Original</option>
              </select>
            </div>
            <div class="te-field">
              <label>Position</label>
              <select id="te-bg-position">
                <option value="center" ${(bg.position||'center')==='center'?'selected':''}>Center</option>
                <option value="top" ${bg.position==='top'?'selected':''}>Top</option>
                <option value="bottom" ${bg.position==='bottom'?'selected':''}>Bottom</option>
                <option value="left" ${bg.position==='left'?'selected':''}>Left</option>
                <option value="right" ${bg.position==='right'?'selected':''}>Right</option>
              </select>
            </div>
          </div>
        </div>
  ...
  function readUIIntoSettings() {
    ...
    const bg = {
      url: (panel.querySelector('#te-bg-url') || {}).value || '',
      opacity: parseFloat((panel.querySelector('#te-bg-opacity') || {}).value || '1'),
      dim: parseFloat((panel.querySelector('#te-bg-dim') || {}).value || '0'),
      blur: parseFloat((panel.querySelector('#te-bg-blur') || {}).value || '0'),
      panelTransparency: parseFloat((panel.querySelector('#te-bg-paneltrans') || {}).value || '0'),
      size: (panel.querySelector('#te-bg-size') || {}).value || 'cover',
      position: (panel.querySelector('#te-bg-position') || {}).value || 'center',
    };
    // If we already have a base64 image stored and the URL field still matches it
    // (user didn't touch the field), keep using the stored base64.
    if (settings.background && settings.background.url &&
        settings.background.url.startsWith('data:') &&
        (panel.querySelector('#te-bg-url') || {}).value === settings.background.url) {
      bg.url = settings.background.url;
    }

    const editorMode = (panel.querySelector('#te-editor-select') || {}).value || 'monaco';

    settings = { colors, background: bg.url ? bg : null, editor: editorMode };
  }
  ...
  function wirePanelEvents() {
    ...
    // Background controls — live preview
    const bgUrlInput = panel.querySelector('#te-bg-url');
    const livePreviewBg = () => {
      const tmp = {
        url: bgUrlInput.value.trim(),
        opacity: parseFloat(panel.querySelector('#te-bg-opacity').value),
        dim: parseFloat(panel.querySelector('#te-bg-dim').value),
        blur: parseFloat(panel.querySelector('#te-bg-blur').value),
        panelTransparency: parseFloat(panel.querySelector('#te-bg-paneltrans').value),
        size: panel.querySelector('#te-bg-size').value,
        position: panel.querySelector('#te-bg-position').value,
      };
      // Preserve stored base64 if field unchanged
      if (settings.background && settings.background.url && settings.background.url.startsWith('data:') &&
          tmp.url === '' ) {
        // leave as-is if user cleared the visible field but a data url is stored separately (rare)
      }
      const prevSettings = settings;
      settings = { ...settings, background: tmp.url ? tmp : null };
      applyBackground();
      settings = prevSettings; // don't persist yet, only preview
    };

    ['#te-bg-opacity', '#te-bg-dim', '#te-bg-blur', '#te-bg-paneltrans'].forEach(sel => {
      const input = panel.querySelector(sel);
      const valEl = panel.querySelector(sel + '-val');
      input.addEventListener('input', () => {
        if (valEl) {
          if (sel === '#te-bg-blur') valEl.textContent = input.value + 'px';
          else valEl.textContent = Math.round(parseFloat(input.value) * 100) + '%';
        }
        livePreviewBg();
      });
    });
    panel.querySelector('#te-bg-size').addEventListener('change', livePreviewBg);
    panel.querySelector('#te-bg-position').addEventListener('change', livePreviewBg);
    bgUrlInput.addEventListener('input', livePreviewBg);

    panel.querySelector('#te-bg-clear-btn').addEventListener('click', () => {
      bgUrlInput.value = '';
      livePreviewBg();
    });

    panel.querySelector('#te-bg-upload-btn').addEventListener('click', () => {
      panel.querySelector('#te-bg-file').click();
    });
    panel.querySelector('#te-bg-file').addEventListener('change', (e) => {
      const file = e.target.files && e.target.files[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = () => {
        bgUrlInput.value = reader.result; // data: URL
        livePreviewBg();
      };
