...
(function () {
  ...
  function injectSidebarButton() {
    ...
    // Insert after sb-webproxy (last button before the spacer)
    const anchor = document.getElementById('sb-webproxy');
    if (!anchor) return;
    ...
    btn.onclick = function () {
      injectSection();
      initCanvas();
      window.switchSection('brickbreaker');
      setTimeout(function () { resizeCanvas(); if (game) render(); }, 30);
    };
    btn.innerHTML =
    ...
    // Insert right after the webproxy button
    anchor.insertAdjacentElement('afterend', btn);
  }

  function injectSection() {
    if (document.getElementById(SECTION_ID)) return;
    // Append directly after section-logs (first section inside main.main)
    const anchor = document.getElementById('section-logs');
    if (!anchor) return;
  ...
  function newBall(g) {
    const paddleY = g.paddle ? g.paddle.y : g.h - 40;
    return {
      x: g.w / 2,
      y: paddleY - 16,
      r: 6,
  ...
  function startOrResume() {
    if (!game) return;
    // If game is over, restart fresh instead of resuming dead state
    if (game.gameOver) {
      restartGame();
      return;
    }
  ...
  function setup() {
    ...
    injectStyles();
    injectSidebarButton();
    // injectSection() is intentionally skipped here — the section is hidden
    // (display:none) so canvas would have 0x0 dimensions. The button click
    // handler calls injectSection() + initCanvas() once it's actually visible.
  }
