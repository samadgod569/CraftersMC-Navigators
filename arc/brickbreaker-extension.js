...
(function () {
  ...
  function injectSidebarButton() {
    ...
    btn.addEventListener('click', () => {
      // Inject the section into DOM right now if not there yet,
      // so switchSection('brickbreaker') won't crash on a missing element.
      injectSection();
      // Init canvas now that the section is actually in the DOM and visible.
      initCanvas();
      // Call the page's own switchSection to handle active classes + hide others.
      if (typeof window.switchSection === 'function') {
        window.switchSection('brickbreaker');
      }
      // Give browser one frame to paint before resizing the canvas.
