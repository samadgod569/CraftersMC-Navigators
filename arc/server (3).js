...
// ─── MONITORING ──────────────────────────────────────────────────────────────
// ports.json entry: [appPass, "IP:port", logs, plan, lastStart, status]
//   plan   : "free" | "premium"
//   status : "active" | "stopped" | "suspended" | "error"
//
// free    — suspend after 24h, remove after 72h
// premium — suspend after 72h, remove after 120h
...
async function runMonitor() {
  ...
  for (const [appName, entry] of Object.entries(data)) {
    const appPass  = entry[0];
    const ipPort   = entry[1];
    const logs     = entry[2];
    ...
    const suspendAfter = plan === "premium" ? MS_72H  : MS_24H;
    const removeAfter  = plan === "premium" ? MS_120H : MS_72H;
    ...
    if (elapsed >= removeAfter) {
      ...
      console.log(`[monitor] REMOVED   ${appName} | plan=${plan} | elapsed=${Math.round(elapsed / MS_1H)}h`);
      continue;
    }

    if (elapsed >= suspendAfter && status !== "suspended") {
      await safeRun("docker", ["stop", appName]).catch(() => {});
      data[appName][5] = "suspended";
      dataDirty = true;
      console.log(`[monitor] SUSPENDED ${appName} | plan=${plan} | elapsed=${Math.round(elapsed / MS_1H)}h`);
    }
  }
...
app.post("/api/deploy", async (req, res) => {
  ...
  if (user.servers.length === 0) {
    ...
    data[appName] = [appPassword, `${VPS_IP}:${newPort}`, [], "free", nowISO(), "active"];
    saveData(data);
  ...
  try {
    ...
    data[appName] = [appPassword, `${VPS_IP}:${port}`, logs, plan, nowISO(), "active"];
    saveData(data);
    ...
    data[appName] = [appPassword, ex[1] || null, logs, ex[3] || "free", ex[4] || nowISO(), "error"];
    saveData(data);
...
app.post("/api/get-app", (req, res) => {
  ...
  user.servers.forEach(appName => {
    if (data[appName]) {
      const [appPass, ipPort, logs, plan, lastStart, status] = data[appName];
      apps[appName] = { appPass, ipPort, logs, plan, lastStart, status };
    }
  });
