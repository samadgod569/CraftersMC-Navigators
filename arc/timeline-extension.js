...
(function () {
  ...
  /* ══════════════════════════════════════════════════════════════════════════
   *  SNAPSHOT STORAGE — real files on the server, not localStorage
   * ══════════════════════════════════════════════════════════════════════════
   *  Each snapshot's content is written as its own file at:
   *    .pylex/extensions/timeline/<urlencoded filePath>/<id>.snap
   *  A single manifest file per app tracks the metadata (id, filePath,
   *  savedAt) for fast listing without re-reading every snapshot file:
   *    .pylex/extensions/timeline/_manifest.json
   *
   *  The manifest is the source of truth for the snapshot LIST; the actual
   *  snapshot content always lives in its own file, read on demand. This
   *  mirrors the same fs/read + fs/save calls the host page already uses,
   *  so snapshots persist server-side instead of in browser localStorage.
   */

  const TL_DIR = '.pylex/extensions/timeline';
  const MANIFEST_PATH = TL_DIR + '/_manifest.json';

  function snapFilePath(id) {
    return TL_DIR + '/' + id + '.snap';
  }
  ...
  async function fsSave(filePath, content) {
  ...
  /**
   * In-memory cache of the manifest so we're not re-fetching it on every
   * render. Always written through (saved to the server) immediately after
   * any mutation, so a page refresh re-reads the authoritative copy.
   */
  let _manifestCache = null;

  async function loadManifest(forceRefresh) {
    if (_manifestCache && !forceRefresh) return _manifestCache;
    try {
      const data = await fsRead(MANIFEST_PATH);
      if (data.status === 'success' && data.content) {
        _manifestCache = JSON.parse(data.content);
      } else {
        _manifestCache = [];
      }
    } catch (_) {
  ...
  async function saveManifest(arr) {
    _manifestCache = arr;
    await fsSave(MANIFEST_PATH, JSON.stringify(arr));
  }

  /** Synchronous accessor for already-loaded manifest entries (UI rendering). */
   ...
   *
   * Writes the snapshot CONTENT to its own file, then updates the manifest
   * with the new entry's metadata. Both go through fs/save — no localStorage.
   */
  async function captureSnapshot(filePath, oldContent) {
    const snap = {
    ...
    const writeRes = await fsSave(snapFilePath(snap.id), oldContent);
    if (writeRes.status !== 'success') {
      throw new Error('Could not write snapshot file: ' + (writeRes.message || 'unknown error'));
    }

    const arr = await loadManifest();
    arr.unshift(snap); // newest first

    // Keep at most 200 snapshots total — delete the overflow's files too,
    // not just drop them from the manifest, or they'd leak on the server.
    const overflow = arr.length > 200 ? arr.splice(200) : [];
    for (const old of overflow) {
      await fsDelete(snapFilePath(old.id));
    }

    await saveManifest(arr);
    return snap;
  }
  ...
  async function deleteSnapshot(id) {
    ...
    await fsDelete(snapFilePath(id));
  }

  /** Reads a snapshot's stored content from its own file (not the manifest). */
  async function readSnapshotContent(id) {
    const data = await fsRead(snapFilePath(id));
    if (data.status !== 'success') throw new Error(data.message || 'Could not read snapshot');
    return data.content || '';
  }
    ...
    try {
      ...
      try {
        ...
        if (readData.status === 'success') {
          ...
          oldContentReadOk = true; // distinguishes "file is empty" from "read failed"
        }
      } catch (_) {
        // If we can't read the old content, we skip the snapshot rather than
        // blocking the save entirely. oldContentReadOk stays false.
      }

      /* ── Step 2: snapshot the old content ─────────────────────────────── */
      // Only the read-success flag (not oldContent !== '') decides whether we
      // got real data — an empty file is a perfectly valid "old content" to
      // snapshot, and treating '' as "no data" silently dropped snapshots.
      if (oldContentReadOk && oldContent !== newContent) {
        try {
          await captureSnapshot(fmCurrentFile, oldContent);
          renderTimelinePanel(); // refresh list
        } catch (snapErr) {
          // Don't block the actual save if snapshotting failed — surface it
          // softly instead.
          tlNotice('Snapshot failed: ' + snapErr.message, 'err');
        }
      }
  ...
  async function renderTimelinePanel() {
    ...
    wrap.innerHTML = `<div class="tl-diff-loading"><span class="tl-diff-spin"></span> Loading snapshots…</div>`;

    let snaps;
    try {
      snaps = await loadManifest(true); // always refresh from server on render
    } catch (e) {
      wrap.innerHTML = `<div class="tl-empty"><p>Couldn't load snapshots: ${escHtml(e.message)}</p></div>`;
      return;
    }
    ...
    for (const [filePath, list] of Object.entries(groups)) {
      ...
      for (const snap of list) {
        const virtualPath = snapFilePath(snap.id);
        html += `
  ...
  async function toggleDiff(id) {
    ...
    try {
      const [snapContent, currentData] = await Promise.all([
        readSnapshotContent(id),
        fsRead(snap.filePath),
      ]);
      if (currentData.status !== 'success') {
        throw new Error(currentData.message || 'Could not read current file');
      }

      const currentContent = currentData.content || '';
      inner.innerHTML = renderDiffHtml(snapContent, currentContent, snap.filePath);
  ...
  async function doRestore(id) {
    ...
    try {
      /* Read the snapshot's actual content from its own file */
      const snapContent = await readSnapshotContent(id);

      /* Write snapshot content back to the original file */
      const data = await fsSave(snap.filePath, snapContent);
      if (data.status !== 'success') throw new Error(data.message);

      /* Delete this snapshot (manifest entry + its content file) */
      await deleteSnapshot(id);

      /* If the editor currently has this file open, reload it */
      if (window.fmCurrentFile === snap.filePath && window._aceEditor) {
        window._aceEditor.setValue(snapContent, -1);
